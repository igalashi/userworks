...
//bool g_flag_dump_module = true;

bool g_flag_only_hits = false;

int CheckDataTdc(uint64_t dword, uint32_t fe_type, uint32_t fe_id)
{
...
//int CheckDataRecbe(uint64_t dword, uint32_t fe_type, uint32_t fe_id)
int CheckDataRecbe(char* raw, uint32_t fe_type, uint32_t fe_id)
{
	//char* raw = reinterpret_cast<char *>(dword);
	//struct Recbe::Header *header = reinterpret_cast<struct Recbe::Header *>(dword);
	struct Recbe::Data data;

	// std::cout << "FE_TYPE: " << fe_type << " ID: " << fe_id << " " << std::endl;
...
int CheckData(char *raw, uint32_t fe_type, uint32_t fe_id)
{
	return CheckDataRecbe(raw, fe_type, fe_id);
}
...
bool CheckBlock(char *buf, int size)
{
	...
	while (true) {
		if (*pdata == SubTimeFrame::MAGIC) {
			...
			if (g_flag_dump_module) CheckData(cdata, fe_type, fe_id);
...
int main(int argc, char* argv[])
{
	...
	for (int i = 1 ; i < argc ; i++) {
		...
		if ((ss == "--only-hit") && (i < argc - 1)) {
			g_flag_only_hits = true;
		} else
