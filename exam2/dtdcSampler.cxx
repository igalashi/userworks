...
bool Sampler::ConditionalRun()
{
	...
	std::vector< std::vector<uint32_t> > tdc;
	make_dummy_dataset(tdc, 32);
	...
	for (size_t i = 0 ; i < tdc.size() ; i++) {
		...
		event_size += (tdc[i].size()) * sizeof(uint32_t);
	}
	char *ebuf = new char[event_size];
	int esize = make_event_buffer(tdc, 1, ebuf);
	assert(event_size != esize);
	static_cast<void>(esize);
	#endif
	
	int event_size = 0;
	char *ebuf = make_event_buffer(tdc, 0x10000, hb, 0, &event_size);
