#ifndef E16_UTILITY_FILE_UTIL_H_
#define E16_UTILITY_FILE_UTIL_H_
...
public:
   ...
   std::ofstream &Get() { return fOut; }
   const std::vector<uint64_t> &GetBranchCompressedSize() const { return fBranchCompressedSize; }
   const std::vector<std::string> &GetBranchFilePath() const { return fBranchFilePath; }
   const std::vector<uint64_t> &GetBranchNumIteration() const { return fBranchNumIteration; }
   const std::vector<uint64_t> &GetBranchRawSize() const { return fBranchRawSize; }
   std::size_t GetBufferSize() const { return fBuffer.size(); }
   int GetCompBufSize() const { return fCompBufSize; }
   CompressFunc GetCompressFunc() const { return fCompressor; }
   const std::string GetDateFormat() const { return fDateFormat; }
   static const std::unordered_map<std::string_view, std::string_view> GetDescriptions();
   const std::string GetExtension() const { return fExt; }
   const std::string GetFilePath() const { return fName; }
   int64_t GetMaxBranchSize() const { return fMaxBranchSize; }
   int64_t GetMaxBranchSizeInMegaBytes() const { return fMaxBranchSize / 1024 / 1024; }
   const std::string GetPrefix() const { return fPrefix; }
   int64_t GetRunNumber() const { return fRun; }
   ...
   void SetBranchNumberFormat(std::string_view f) { fBranchNumberFormat = f.data(); }
   void SetBufferSize(int n) { fBuffer.resize(n); }
   void SetCompBufSize(int n) { fCompBufSize = n; }
   void SetCompressFunc(CompressFunc f) { fCompressor = f; }
   void SetDateFormat(std::string_view f) { fDateFormat = f.data(); }
   void SetExtension(std::string_view ext);
   void SetFilePath(std::string_view path) { fName = path; }
   void SetMaxBranchSize(int64_t n) { fMaxBranchSize = n; }
   void SetMaxBranchSizeInMegaBytes(int64_t n) { SetMaxBranchSize(n * 1024 * 1024); }
   bool SetOpenmode(std::string_view mode);
   void SetPermissions(std::string_view p);
   void SetPrefix(std::string_view prefix) { fPrefix = prefix.data(); }
   void SetRunNumber(int64_t runNumber) { fRun = runNumber; }
   void SetRunNumberFormat(std::string_view f) { fRunNumberFormat = f.data(); }
...
#endif // E16_UTILITY_FILE_UTIL_H_H
