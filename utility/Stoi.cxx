...
int e16::GetBase(std::string_view arg, int debug)
{
   auto base = 0;
   if (debug > 0) {
...
//______________________________________________________________________________
uint64_t e16::Stoi(std::string_view arg, int debug)
{
   auto base = GetBase(arg, debug);
   if (base <= 0) {
      throw std::runtime_error(std::string("e16daq::Stoi() arg is not digit: ") + arg.data());
   }
